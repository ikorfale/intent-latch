...
export async function dispatch(intent, digest, {
  ...
  const headers = Object.freeze({
    ...
    'User-Agent': 'IntentLatch/1.0 (+https://intent-latch-two.vercel.app/security)',
    'Content-Length': String(body.length),
    // Keep retries and repeated prepares for one logical request in the same
    // destination idempotency domain. The full intent digest remains a
    // separate integrity binding below.
  ...
  const options = Object.freeze({
    ...
    lookup(_host, lookupOptions, callback) {
      if (lookupOptions?.all) callback(null, [vetted]);
