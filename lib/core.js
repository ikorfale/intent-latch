  ...
  try {
    ...
    return {
      ...
      review: {
        ...
        authority_limit: 'The deterministic digest confirms bytes only; it does not authenticate or authorize the caller. Hosted v1 is harmless-demo-only.',
        warning: 'Prepare URL input is public and may appear in browser, CDN, or Vercel logs. Never include secrets or personal data.',
        idempotency_limit: 'The stable request_id key only helps when the destination atomically binds it to the first body/digest and rejects conflicts. A timeout means outcome unknown.'
      },
      commit_template: {
        method: 'POST',
